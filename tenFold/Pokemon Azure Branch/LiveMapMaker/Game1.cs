...
namespace LiveMapMaker
{
    ...
    public class Game1 : Microsoft.Xna.Framework.Game
    {
        ...
        private void findMouseoverTile()
        {
            ...
            if(xnaForm.Focused)
            {
                ...
                for (int x = 0; x < world.currentArea.mapWidth; x++)
                {
                    ...
                    {
                        float distance = (world.currentArea.tile[x, y].Z - cursorRay.Position.Y) / cursorRay.Direction.Y; //Y is "height" when drawing
            ...
            form.updateDebug(selectedX + ", " + selectedY);
        }
        ...
        private void drawSelectedBorder()
        {
            ...
            {
                Vector3 position = new Vector3((world.currentArea.globalX + selectedX) * 1, world.currentArea.tile[selectedX, selectedY].Z, (world.currentArea.globalY + selectedY) * 1);
            ...
            if (s.size.X > 0 && s.size.Y > 0)
            {
                ...
                for (int x = 0; x < s.size.X; x++)
                {
                    for (int y = 0; y < s.size.Y; y++)
                    {
                        ...
                        {
                            Vector3 position = new Vector3((world.currentArea.globalX + selectedX + x) * 1, world.currentArea.tile[selectedX + x, selectedY + y].Z, (world.currentArea.globalY + selectedY + y) * 1);
        ...
        private void drawAccessibility()
        {
            ...
            for(int x = 0; x < world.currentArea.mapWidth; x++)
            {
                for(int y = 0; y < world.currentArea.mapHeight; y++)
                {
                    ...
                    Vector3 position = new Vector3((world.currentArea.globalX + x) * 32, world.currentArea.tile[x, y].Z, (world.currentArea.globalY + y) * 32);
                    pointList[0] = new VertexPositionColor(position + new Vector3(-0.46f, 0f, -0.46f), Color.Red);
                    pointList[1] = new VertexPositionColor(position + new Vector3(0.46f, 0.46f, -0.46f), Color.Red);
            ...
            for (int x = 0; x < world.currentArea.mapWidth; x++)
            {
                for (int y = 0; y < world.currentArea.mapHeight; y++)
                {
                    ...
                    if (world.currentArea.tile[x, y].eventScript != null && world.currentArea.tile[x, y].eventScript != "")
                    {
                        ...
                        Vector3 position = new Vector3((world.currentArea.globalX + x) * 1, world.currentArea.tile[x, y].Z, (world.currentArea.globalY + y) * 1);
