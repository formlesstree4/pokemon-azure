...
namespace PokeEngine.Screens
{
    public static class GameDraw
    {
        ...
        public static Vector3 cameraOffset = new Vector3(0.0f, 10.0f, 6.0f); //distance from player the camera is
            ...
            foreach (Zone z in inWorld.adjacentAreas)
            {
                ...
                for (int x = 0; x < z.mapWidth; x++)
                {
                    for (int y = 0; y < z.mapHeight; y++)
                    {
                        ...
                        Vector3 TL = new Vector3(xOffset + (float)x * 1f - 0.5f, height, yOffset + (float)y * 1f - 0.5f);
                        Vector3 TR = new Vector3(xOffset + (float)x * 1f + 0.5f, height, yOffset + (float)y * 1f - 0.5f);
                        Vector3 BR = new Vector3(xOffset + (float)x * 1f + 0.5f, height, yOffset + (float)y * 1f + 0.5f);
                        Vector3 BL = new Vector3(xOffset + (float)x * 1f - 0.5f, height, yOffset + (float)y * 1f + 0.5f);
        ...
        public static void DrawScenery(World inWorld)
        {
            ...
            if (drawAdjObjects)
            {
                foreach (Zone z in inWorld.adjacentAreas)
                {
                    ...
                    {
                        DrawScenery(s, z.globalX, z.globalY);
                    }
                }
            }
            else
            {
                ...
                {
                    DrawScenery(s, inWorld.currentArea.globalX, inWorld.currentArea.globalY);
                }
            }
        }
        ...
        public static void DrawScenery(Scenery s, int x, int y)
        {
            ...
            {   
                ...
                foreach (ModelMesh mesh in mod.Meshes)
                {
                    ...
                    effect.Parameters["World"].SetValue(transforms[mesh.ParentBone.Index] * s.rotation * s.scale * s.translation * Matrix.CreateTranslation(new Vector3(x + s.position.X - 0.5f, 0,y + s.position.Y - 0.5f)));
                    effect.Parameters["View"].SetValue(view);
                    effect.Parameters["Projection"].SetValue(projection);
