...
namespace Mercury.Database
{
    ...
    static class SearchTracker
    {
        ...
        private static Dictionary<Guid, List<Classes.SearchPkm>> _searches;

        public static void Initialize()
        {
            _searches = new Dictionary<Guid, List<Classes.SearchPkm>>();
        }

        public static void Add(Guid searchid, List<Classes.SearchPkm> pokemon)
        {
            if (!_searches.ContainsKey(searchid)) _searches.Add(searchid, pokemon);
        }
        public static List<Classes.SearchPkm> GetResults(Guid searchID)
        {
            ...
            if (!_searches.ContainsKey(searchID)) return new List<Classes.SearchPkm>();

            // now that we have done that, grab the current list.
            var currentList = _searches[searchID];
            ...
            if (currentList.Count > Constants.MaxResults)
            {
                ...
                {
                    _searches[searchID] = currentList;
                }
